...
export function Timeline() {
	...
	return (
		<div
			className={
				"relative flex h-full flex-col overflow-hidden rounded-sm bg-panel transition-colors duration-200"
			}
			{...dragProps}
			onMouseEnter={() => setIsInTimeline(true)}
			onMouseLeave={() => setIsInTimeline(false)}
		>
			<TimelineToolbar setZoomLevel={setZoomLevel} zoomLevel={zoomLevel} />
			...
			>
				...
				<div className="sticky top-0 z-10 flex bg-panel">
					...
					<div className="flex w-48 flex-shrink-0 items-center justify-between border-r bg-muted/30 px-3 py-2">
						{/* Empty space */}
						<span className="font-medium text-muted-foreground text-sm opacity-0">
							.
						</span>
					</div>
				...
				<div className="flex flex-1 overflow-hidden">
					...
					{tracks.length > 0 && (
						<div
							className="w-48 flex-shrink-0 overflow-y-auto border-black border-r"
							data-track-labels
							ref={trackLabelsRef}
						>
							<ScrollArea className="h-full w-full" ref={trackLabelsScrollRef}>
								<div className="flex flex-col gap-1">
									{tracks.map((track) => (
										<div
											className="group flex items-center border-muted/30 border-b bg-foreground/5 px-3"
											key={track.id}
		...
		<div className="flex items-center justify-between border-b px-2 py-1">
			<div className="flex w-full items-center gap-1">
				<TooltipProvider delayDuration={500}>
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={handleSplitSelected} size="icon" variant="text">
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={handleSeparateAudio} size="icon" variant="text">
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={handleFreezeSelected} size="icon" variant="text">
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={handleDeleteSelected} size="icon" variant="text">
			...
			<div className="flex items-center gap-1">
				<TooltipProvider delayDuration={500}>
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={toggleSnapping} size="icon" variant="text">
								{snappingEnabled ? (
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button onClick={toggleRippleEditing} size="icon" variant="text">
								<Link
				...
				<div className="flex items-center gap-1">
					<Button onClick={handleZoomOut} size="icon" variant="text">
					...
					<Slider
						...
						value={[zoomLevel]}
					/>
					<Button onClick={handleZoomIn} size="icon" variant="text">
